...
impl CommonFxAssets {
    /// Select a sound for something impacting against the given material.
    pub fn select_sound_for_surface_impact(&self, mat: SurfaceMaterial) -> Option<Handle<AudioSample>> {
        let fx = self;

        let mats: &[&Handle<AudioSample>] = match mat {
            SurfaceMaterial::Unknown | SurfaceMaterial::Synthetic => &[
                &fx.thump_1a,
                &fx.thump_2a,
                &fx.thump_3a,
                &fx.thump_4a,
        ...
        (*mats).choose(&mut rand::rng()).map(|a| (*a).clone())
    }

    /// Select a sound for something sliding against the given material.
    pub fn select_sound_for_surface_slide(&self, mat: SurfaceMaterial) -> Option<Handle<AudioSample>> {
        let fx = self;

        let mats: &[&Handle<AudioSample>] = match mat {
            SurfaceMaterial::Unknown | SurfaceMaterial::Synthetic | SurfaceMaterial::Fabric => &[
                &fx.brush1a,
                &fx.brush1b,
                &fx.brush1c,
                &fx.brush1d,
    ...
    /// Select a footstep sound for the given surface.
    pub fn select_sound_for_footstep(&self, mat: SurfaceMaterial) -> Option<Handle<AudioSample>> {
        let fx = self;
